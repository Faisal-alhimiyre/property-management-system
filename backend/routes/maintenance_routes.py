...
async def create_maintenance_request(request: MaintenanceRequest, current_user: dict = Depends(get_current_user)):
    ...
    tenant = supabase.table("tenants").select("*").eq("user_id", current_user["id"]).eq("apartment_id", request.apartment_id).execute()
    if not tenant.data:
...
async def update_maintenance_request(request_id: int, status: str, current_user: dict = Depends(get_current_user)):
    ...
    response = supabase.table("maintenance_requests").update({"status": status, "updated_at": "now()"}).eq("id", request_id).execute()
    return response.data[0]